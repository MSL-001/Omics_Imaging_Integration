  ...
  for (rep_i in seq_len(nrepeat)) {
    ...
    for (fold_i in seq_len(folds)) {
      ...
      for (comp_i in seq_len(ncomp_max)) {
        ...
        obs_test <- as.matrix(y_test)
        est_test <- as.matrix(y_pred[, , 1])
        obs_train <- as.matrix(y_train)
        est_train <- as.matrix(y_pred_train[, , 1])
...
run_modeling <- function (prot, img){
  ...
  meta_img_aligned <- meta_data[match(img$eid, meta_data$eid), ]

  img <- img[, setdiff(names(img), "eid"), drop = FALSE]

  img_train_mat <- img[, setdiff(names(img), "eid"), drop = FALSE]

  img_resid_models <- vector("list", ncol(img_train_mat))
  names(img_resid_models) <- colnames(img_train_mat)

  img_resid <- img_train_mat

  for (j in seq_along(img_train_mat)) {
    res_obj <- fit_residualizer(
      v = img_train_mat[[j]],
    ...
    img_resid[[j]] <- res_obj$residuals
    img_resid_models[[j]] <- res_obj
  }

  img_resid$eid <- img_eids

  img <- img_resid

  saveRDS(prot_resid_models, paste0(interested_feature, "_protein_residualizers.rds"))
  saveRDS(img_resid_models,     paste0(interested_feature, "_image_residualizers.rds"))
  ...
  y <- img[, setdiff(names(img), "eid"), drop = FALSE]
