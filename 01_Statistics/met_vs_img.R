...
run_modeling <- function (met, img){
  ...
  met_train_mat <- met[, setdiff(names(met), "eid"), drop = FALSE]

  met_resid_models <- vector("list", ncol(met_train_mat))
  names(met_resid_models) <- colnames(met_train_mat)

  met_resid <- met_train_mat

  for (j in seq_along(met_train_mat)) {
    ...
    met_resid[[j]] <- res_obj$residuals
    met_resid_models[[j]] <- res_obj
  }

  met_resid$eid <- met_eids
  ...
  saveRDS(met_resid_models, paste0(interested_feature, "_metabolite_residualizers.rds"))
  ...
  saveRDS(norm_met, paste0(interested_feature, "_metabolite_normalizers.rds"))
  ...
  pls.result <- pls(X, y, ncomp= 20, mode = 'regression')
